	...
	return (
		<motion.nav
			...
			<div className=' container relative max-w-screen-2xl flex gap-4 top-full p-2 md:pb-6 md:pt-10 lg:pr-32 justify-end  text-base-content'>
				<ul
					className={` absolute md:static min-h-screen md:min-h-min flex flex-col md:flex-row items-center  p-4 pt-36 md:pt-0 text-center gap-14 transition-all duration-300 top-0 right-0 w-full lg:w-auto bg-base-200 md:bg-opacity-0 ${
						isOpen ? ' ' : ' translate-x-full md:translate-x-0'
					}`}>
					<li onClick={closeMenu}>
						<Link href={'/calls'}>Звонки</Link>{' '}
					</li>
					<li onClick={closeMenu}>Администраторы</li>
				</ul>
				<MenuToggle
